...
/**
 * Simple utility wrapper class to represent the persistent object identity ID
 * numbers. Use the getHigh() and getLow() methods individually to get each ID,
 * or the combined hex string through toString().
 * 
...
public class ITObjectPersistentID {
	...
	public ITObjectPersistentID(long high, long low) {
		...
		this.hexString = String
				.format("%8s%8s", Long.toHexString(this.High),
						Long.toHexString(this.Low)).toUpperCase()
				.replace(' ', '0');
	}
