...
public class ITUserPlaylist extends ITPlaylist {
	...
	/**
	 * Add a file or files inside a folder to the playlist. You cannot use this
	 * method to add a file that requires conversion to be added (e.g. a CD
	 * track), use <code>iTunes.convertFile()</code> or
	 ...
	 * 
	 * @param filePath
	 *            The full path to the file or folder to add.
	 * @return Returns an ITOperationStatus object corresponding to the
	 *         asynchronous operation. If an error occurs, or no files were
	 *         added, this will be set to <code>NULL</code>.
	 */
	 ...
	 * 
	 * @param url
	 *            The URL to add. The length of the URL can be 255 characters or
	 *            less.
	...
	/**
	 * Add an existing track to the playlist. You cannot use this method to add
	 * a CD track (ITTrackKindCD) to another playlist, use
	 * <code>iTunes.convertTrack()</code> or <code>iTunes.convertTrack2()</code>
	 * instead. You cannot add a shared library track (ITTrackKindSharedLibrary)
	 * to another playlist.
	 * 
	 * @param track
	 *            The track to add.
	...
	public ITTrack addTrack(ITTrack track) {
		...
		Dispatch addedTrack = Dispatch.call(object, "AddTrack", trackToAdd)
				.toDispatch();
