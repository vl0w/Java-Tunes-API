...
public class iTunes {
	 ...
	 * 
	 * @param itef
	 *            The class that will handle the iTunes events.
	 */
	 ...
	 * targeted track, returns the current state of the player buttons in the
	 * main browser window.
	 */
	 ...
	 * 
	 * @param majorVersion
	 *            Major version of iTunes interface.
	 * @param minorVersion
	 *            Minor version of iTunes interface.
	...
	public boolean getCheckVersion(int majorVersion, int minorVersion) {
		return iTunes.invoke("CheckVersion", majorVersion, minorVersion)
				.getBoolean();
	}

	/**
	 * Returns an IITObject corresponding to the specified IDs. The object may
	 * be a source, playlist, or track.
	 * 
	 * @param sourceID
	 *            The ID that identifies the source. Valid for a source,
	 *            playlist, or track.
	 * @param playlistID
	 *            The ID that identifies the playlist. Valid for a playlist or
	 *            track. Must be zero for a source.
	 * @param trackID
	 *            The ID that identifies the track within the playlist. Valid
	 *            for a track. Must be zero for a source or playlist.
	 * @param databaseID
	 *            The ID that identifies the track, independent of its playlist.
	 ...
	 */
	public ITObject getITObjectByID(int sourceID, int playlistID, int trackID,
			int databaseID) {
		Dispatch object = Dispatch.call(iTunes, "GetITObjectByID", sourceID,
				playlistID, trackID, databaseID).toDispatch();
	 ...
	 * 
	 * @param playlistName
	 *            The name of the new playlist (may be empty).
	...
	public ITPlaylist createPlaylist(String playlistName) {
		Dispatch cplaylist = Dispatch.call(iTunes, "CreatePlaylist",
				playlistName).toDispatch();
	 ...
	 * 
	 * @param url
	 *            The URL to open. The length of the URL cannot exceed 512
	 *            characters. iTunes Store URLs start with itms:// or itmss://.
	 *            Streaming audio URLs start with http://.
	 */
	...
	/**
	 * Creates a new EQ preset. The EQ preset will be created "flat", i.e. the
	 * preamp and all band levels will be set to 0. EQ preset names cannot start
	 * with leading spaces. If you specify a name that starts with leading
	 * spaces they will be stripped out. If <code>eqPresetName</code> is empty,
	 * the EQ preset will be created with a default name.
	 * 
	 * @param eqPresetName
	 *            The name of the new EQ Preset (may be empty)
	...
	public ITEQPreset createEQPreset(String eqPresetName) {
		Dispatch eqPreset = Dispatch.call(iTunes, "CreateEQPreset",
				eqPresetName).toDispatch();
		return new ITEQPreset(eqPreset);
	}

	/**
	 * Creates a new playlist in an existing source. You may not be able to
	 * create a playlist in every source. For example, you cannot create a
	 * playlist in an audio CD source, or in an iPod source if it is in auto
	 * update mode. If <code>playlistName</code> is empty, the playlist will be
	 * created with a default name.
	 * 
	 * @param playlistName
	 *            The name of the new playlist (may be empty).
	 * @param source
	 *            The source that will contain the new playlist.
	 * @return Returns an ITPlaylist object corresponding to the new playlist.
	 */
	public ITPlaylist createPlaylistInSource(String playlistName,
			ITSource source) {
		Dispatch cplaylist = Dispatch.call(iTunes, "CreatePlaylistInSource",
				playlistName, source.fetchDispatch()).toDispatch();
		ITPlaylist playlist = new ITPlaylist(cplaylist);
		ITPlaylistKind playlistKind = playlist.getKind();
		if (playlistKind == ITPlaylistKind.ITPlaylistKindCD)
			return new ITAudioCDPlaylist(cplaylist);
	...
	/**
	 * Creates a new folder in the main library. If <code>folderName</code> is
	 * empty, the folder will be created with a default name.
	 * 
	 * @param folderName
	 *            The name of the new folder (may be empty).
	 * @return Returns an ITPlaylist object corresponding to the new folder.
	 */
	public ITUserPlaylist createFolder(String folderName) {
		Dispatch folder = Dispatch.call(iTunes, "CreateFolder", folderName)
				.toDispatch();
		return new ITUserPlaylist(folder);
	}

	/**
	 * Creates a new folder in an existing source. You may not be able to create
	 * a folder in every source. For example, you cannot create a folder in an
	 * audio CD source, or in an iPod source if it is in auto update mode. If
	 * <code>folderName</code> is empty, the folder will be created with a
	 * default name.
	 * 
	 * @param folderName
	 *            The name of the new folder (may be empty)
	 * @param iSource
	 *            The source that will contain the new folder.
	 * @return Returns an ITPlaylist object corresponding to the new folder.
	 */
	public ITUserPlaylist createFolderInSource(String folderName,
			ITSource iSource) {
		Dispatch folder = Dispatch.call(iTunes, "CreateFolderInSource",
				folderName, iSource.fetchDispatch()).toDispatch();
		return new ITUserPlaylist(folder);
	}
	 ...
	 * 
	 * @param shouldMute
	 *            If true, sound output will be muted.
	 */
	...
	public ITPlayerState getPlayerState() {
		return ITPlayerState.values()[Dispatch.get(iTunes, "PlayerState")
				.getInt()];
	}

	/**
	 * Sets the player's position within the currently playing track in seconds.
	 * If playerPos specifies a position before the beginning of the track, the
	 * position will be set to the beginning. If playerPos specifies a position
	 * after the end of the track, the position will be set to the end.
	 * 
	 * @param playerPos
	 *            The player's position within the currently playing track in
	 *            seconds.
	 */
	...
	/**
	 * Returns the source that represents the main library. You can also find
	 * the main library source by iterating over
	 ...
	 *         currently targeted track. Will be set to NULL if there is no
	 *         currently targeted playlist.
	 */
	 ...
	 * 
	 * @return Collection of ITrack objects. Will be set to NULL if there is no
	 *         current selection.
	 */
	...
	public ITObjectPersistentID getObjectPersistentIDs(ITObject iObject) {
		return new ITObjectPersistentID(getITObjectPersistentIDHigh(iObject),
				getITObjectPersistentIDLow(iObject));
	}
